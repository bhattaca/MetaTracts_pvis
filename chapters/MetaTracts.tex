...
\includegraphics[width=0.5\textwidth]{imagesMT2014/image3.png}
...
\begin{figure}[tb]
 ...
 \caption{MetaTracts generation: (B) MetaTract generation, region with $R_H$ less than threshold. (A) close up of marked region in (B). (C) Parts of a single MetaTract.}
  \label{fig:algo}
\end{figure}
The set of vertices which are in the blue region but not overlapped by green are possible candidates for the start point of the next cylinder in a particular MetaTract. We call these vertices \textit{``candidate vertices"}. From these we select another grid vertex which will be the start point for the next cylinder.
%\begin{figure}
%\centering
...
\begin{figure}[tb]
 ...
 %\includegraphics[width=0.45\textwidth]{imagesMT2014/crop-16/orientation_cluster.PNG}
  \includegraphics[width=0.45\textwidth]{imagesMT2014/image6}
...
\begin{figure*}
\centering
		\includegraphics[width=0.4\textwidth]{imagesMT2014/garb1}~
		\includegraphics[width=0.4\textwidth]{imagesMT2014/image7b}
...
Fig.~\ref{fig:prepreg} shows the results on our dataset2. This dataset is dense with \textit{flat and thin} bundles. The dimensions are 300x350x300 and the datatype is uint16. The bundles are indistinguishable in the original data set Fig.~\ref{fig:prepreg}(A). A green dotted line shows one of the fiber bundle boundary. Fig.~\ref{fig:prepreg}(B) shows the fiber bundles for the two orientation. Fig.~\ref{fig:prepreg}(C) shows the combined results.Fig.~\ref{fig:prepreg}(D) shows the volume after voxelization.
