...
%REPLACED BY 
%Carbon fiber reinforced polymers (CFRP) are promising materials for future components as they allow integrating the continuously rising demands of various industrial  branches.
...
The general assumption of fiber clustering in diffusion tensor imaging is that a similarity measure based on factors such as proximity between fibers can be used to interpret a collection of fiber tracts as bundles.  Extensive research has been done on automatic DTI fiber clustering methods (see \cite{Corouge2004},~\cite{Brun2003},~\cite{Brun2004},~\cite{Zhang2008},~\cite{westinMEDIA02}). We limit ourselves on advancement of interest to our current work.
 %Zhang et al.~\cite{Zhang2008}, O'Donnell et al.~\cite{westinMEDIA02}. 
...
\begin{figure}[h]
  \begin{center}
  \includegraphics[ width=0.9\linewidth]{imagesMT2014/MT_abstract_A2} %algo-1d}
  \end{center}
...
\end{equation}
 ...
 In this particular example Figure~\ref{fig:algo}B we select the orange grid vertex next and repeat the process. We do not select the purple vertex (Figure~\ref{fig:algo}C) because it is not a candidate vertex. If we generate tracts that have erroneous local orientations they will not be able to find further possible candidate vertices inside the cylinder and will be of short length. Short tracts are then removed using a threshold on MetaTracts  length. The MetaTracts generated are shown in Figure~\ref{fig:length_distribution}(a). The MetaTracts are colored with the mean orientation direction mapped to the RGB space. That consistent orientation is a key intrinsic feature in the data becomes visually pronounced.
...
\begin{figure}[h]
\centering
	\subfloat[]{\includegraphics[width=0.49\linewidth]{imagesMT2014/MT_crop16_MT.png}}
	\subfloat[]{\includegraphics[width=0.49\linewidth]{imagesMT2014/length_distribution.PNG}}
...
%\begin{figure}
%\centering
%\subfloat[]{\includegraphics[width=0.25\linewidth]{imagesMetaTracts/glass-scalar.eps}}
%  \subfloat[]{\includegraphics[width=0.25\linewidth]{imagesMetaTracts/glass-scalar-slice.eps}}
%  \subfloat[]{\includegraphics[width=0.25\linewidth]{imagesMetaTracts/glass-cluster-b.eps}}
%  \subfloat[]{\includegraphics[width=0.25\linewidth]{imagesMetaTracts/glass-cluster-a.eps}}
%  \caption{Our algorithm on a data set made of glass fibers. (a) The scalar dataset, (b) Depicts a slice from the data set, the  fibers are clearly demarcated from the rest of the matrix as oppose to the type of data  where it is hard to distinguish the fibers (Fig. \ref{fig:orig_data}). (c) The result of the MetaTracts process, the colors are to help visualization. (d) The result of the orientation based clustering, the two clusters are marked red and yellow.}\label{fig:glass}
%\end{figure}
...
%\textit{Glass Fibers}, While the core advantages of our approach are in cases when the individual fibers are not discernible, our approach also works on data where the fibers are relatively well separated. Fig. \ref{fig:glass} shows our framework applied on a glass  data set (350x350x300 in size) with an isotropic resolution of $2 \mu m$. Fig. \ref{fig:glass}b shows a slice from the original dataset, (comparison with Fig. \ref{fig:orig_data} shows the individual fiber separation is easier in this case.) 
% Fig.~\ref{fig:glass}c shows the result of the MetaTracts extraction process. Fig~\ref{fig:glass}d shows the results of the orientation based clustering into two clusters (reds and yellows). TODO: with this as the ground truth, add noise and repeat experiment, to show robustness.
...
%\begin{figure}
%  \centering
%\includegraphics[width=0.2\linewidth]{imagesMetaTracts/compare-clus-1.eps}
%\includegraphics[width=0.2\linewidth]{imagesMetaTracts/compare-clus-2.eps}
%  \caption{Comparison of clustering approaches.(a) Orientation clustering into 2 classes followed by hierarchical clustering into 3 classes each. (b)Hierarchical clustering into 6 classes.}
%  
% \label{fig:fiber-clus-2}
%\end{figure}
...
%\textbf{weighted undirected graph}
%In DT-MRI, each voxel is assigned a tensor that describes local water diffusion. In our case the Hessian is error prone
