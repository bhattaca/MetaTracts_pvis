...
In general, CFRP materials show highly desired characteristics as high specific stiffness, high specific strength, high corrosion resistance at considerably lower weight compared to conventional materials. At the same time using CFRPs highly complex and integrated components may be produced, which were previously impossible to manufacture. For example even primary structures and highly loaded components have been increasingly manufactured from CFRP in recent years.
...
%short fiber reinforced composites as presented by Weissenbock2014 et al.~\cite{Weissenbock2014}.
Figure~\ref{fig:data-char} provides an example of our targeted datasets. It also shows an example of the recurring fiber bundle pattern, called a \textit{unit cell}.
Our work is motivated from the recent progress in two interrelated fields: On the one hand, CFRP components have gained wide application in a variety of industries  because of its superior material and physical properties in comparison to conventional materials \cite{Karpat2012}. On the other hand the use of industrial 3D X-Ray computed tomography (XCT) based non destructive testing of fiber reinforced components has increasingly gained popularity~\cite{Schilling2005}. 
...
\begin{itemize}
\item{ How to extract and visualize the geometric structure of a particular bundle?}
