...
\label{sec:intro}
Modern industry is increasingly demanding function orientation, integration, and efficiency of novel materials and components. The material of choice for a growing number of applications \replacedWith{are}{is} carbon fiber reinforced polymer (CFRP), which \replacedWith{allow}{allows} an integration of \replacedWith{those}{these} continuously rising demands \added{and increasingly replaces conventional materials such as aluminum or steel.} 
\replacedWith
{
...
\replacedWith
{
In this work, we focus on datasets with larger viewports and
}
{
...
Figure~\ref{fig:data-char} \replacedWith{provides an example of}{depicts} our targeted dataset \added{type}. It \removed{also} shows the recurring fiber bundle pattern in the final CFRP laminate, \replacedWith{called a}{the} \textit{unit cell}.
Our work is motivated from the recent progress in two interrelated fields: \replacedWith{On one hand}{Firstly}, CFRP components have gained wide application in industry because of its superior material and physical properties in comparison to conventional materials~\cite{Karpat2012}. \replacedWith{On the other hand}{Secondly}, recent developments of industrial 3D X-ray computed tomography (XCT) with regard to larger detectors, larger field of views, and better resolutions opened XCT for this new application area of \replacedWith{non destructive testing}{NDT} for fiber reinforced components~\cite{Schilling2005}. 
%
...
%
While fiber bundles are now understood as highly important in determining component properties, the tools for \replacedWith{visualization of}{visualizing} the internal structure have not developed at the same pace.
...
\item{ Which fiber bundles are of the same type of yarn? I.e. which bundles show similar sizes or diameters, which is the largest or smallest fiber bundle\added{?}}
\end{itemize}

We demarcate the above queries into two parts: \textit{geometric structure} and \textit{spatial context}. Geometric structure refers to the shape, size, and orientation of a fiber bundle or a group of \replacedWith{bundles}{them}. Spatial context refers to how two or more bundles interact with each other. \replacedWith{For example, spatial context}{It provides} answers \added{to} questions such as: Are these bundles in contact at a particular position in the data? What are the relative orientations of the contacting bundles? 
\replacedWith{Answering}{Providing answers to} the above queries from the volume renderings of XCT datasets or from visual inspection of particular 2D slices is non-trivial, even for experts. We \replacedWith{detail}{present} and evaluate a method which uses visualization techniques to gain insight into our data.
We \replacedWith{advance and interpret}{interpret and advance} techniques from diffusion tensor imaging to extract and visualize geometric structures from \replacedWith{3D X-ray computed tomography}{XCT} data of the woven carbon fiber reinforced composites. The main goal of this work is to expand the state of the art in non-destructive testing through visualization of composite structures in \replacedWith{lower resolutions and thus larger areas}{\textit{complete unit cells}} of woven fabrics.
\begin{figure}[tb]
	\centering
...
Because decisions are made locally, these methods perform poorly in noisy regions and often generate small fibers. Basser et al.~\cite{Basser2002} proposed that white matter tracts could be represented as 3D curves in space. They showed that numerical methods could be used to follow fibers and fiber bundles and to generate tracts in human brain data. 
...
In DTI, similarity measures such as proximity between fibers are used to group fiber tracts into bundles. Extensive research has been done on automatic DTI fiber clustering methods \cite{Brun2004,Brun2003,Corouge2004,westinMEDIA02,Zhang2008}. 
Clustering requires choosing a suitable proximity measure and \removed{choosing} a method for grouping ``close" fibers.
Pairwise proximity measures include endpoint distances~\cite{Brun2003} and mean of the closest distances between points on two fibers~\cite{Corouge2004}. Zhang et al.~\cite{Zhang2008} introduced a thresholded version of the closest distances mean, so that fibers \replacedWith{that}{which} are close for certain distance and then diverge, are clustered separately. Brun et al.~\cite{Brun2004} use\removed{s} normalized cuts along with a pairwise distances measure computed using a 9\removed{-}D fiber shape descriptors. The choice of the clustering algorithm can be broadly divided into those approaches using hierarchical clustering~\cite{Moberts2005, Zhang2008} and those using spectral clustering~\cite{Brun2004,jonasson2005, ODonnell2007}.
...
There are a number of difficulties in hierarchical clustering. First, computing all pairs\added{'} distances for tracts to generate the distance matrix is time consuming~\cite{Garyfallidis2012}. Second, a ``correct'' distance measure to compare tracts must be chosen. Third, hierarchical clustering is best suited for similar length fibers.
...
%To the best of our knowledge there is no approach focusing on direct extraction of fiber bundle structures from low resolution XCT of composite fibers.
...
\replacedWith
{
Data characteristics: (a) Rendering of the data. (b) 2D slices along the X-axis and (c) 2D slice along the Z-axis. (d) Magnified region marked in green square (c). Multiple fiber bundles cross and are indistinguishable by visual inspection alone. 
}
{
...
\item Local orientation: each point \removed{with}in a fiber bundle has a local orientation \removed{which is} parallel to the corresponding area in the fiber bundle.
\item \replacedWith{The l}{L}ocal orientation may \removed{change} gradually \replacedWith{inside}{change in} the fiber bundle.
\item \replacedWith{The l}{L}ocal orientation may be noisy and not reliable.
...
\label{sec:approach}
Extracting the MetaTracts consists of two  major steps. First\added{,} we extract a local orientation vector at each grid location (Sec.~\ref{subsec:rh}). Second, we compute coarse poly-cylinders called MetaTracts which traverse the data (Sec.~\ref{subsec:mtprop}).
...
The input to the first stage of our pipeline, is the original scalar dataset in a uniform lattice grid in $\mathbb{R}^3$. As output, each grid location \replacedWith{becomes}{gets} associated with an unit vector representing the local orientation and a real value [0,1] that represents a measure of the reliability of the local orientation. We approximate the local orientation by eigenvalue analysis of the Hessian matrix computed at each voxel.
The Hessian matrix represents the local curvature. The eigen decomposition of the Hessian matrix gives the eigenvectors which describe the local second-order structure of the image. The eigenvector corresponding to the smallest eigenvalue (also referred as the principal direction) gives the direction along which the curvature is smallest. This direction coincides with the direction of the tubular structure. \replacedWith
...
\end{equation}
In Equation~\ref{RB}, $\mathcal{R_{B}}$ provides a measure of deviation from a blob like structure while in Equation~\ref{RA}, $\mathcal{R_{A}}$ distinguishes between plate-like and line-like structure. Grey-scale variations and close proximity of the fibers in our data make the Hessians computed at each voxels susceptible to errors. Thus, we compute \replacedWith{the ``vesselness'' measure}{reliable Hessians ($R_H$)} to determine which locations in the volume provide reliable local orientation. 
$$
...
{We also note here some intrinsic differences between the DTI and our XCT data. Fiber traces can be created in DTI using a standard fiber tracking algorithm following the principal direction of diffusion using a fourth order Runge-Kutta method~\cite{Brun2003}. The principal direction based on the Hessian matrix works best when the tubular structures are well separated from the surrounding. This is not the case for our data. The local orientations at each voxel are inherently noisier.}
...
\includegraphics[width=\linewidth]{images_pvis/reliable_hessian.pdf}
 \vspace{-1.5em}
\caption{Reliable Hessians. (a) Colored according to Orientation vector mapped to RGB. (b\added{,c}) 2D slice\added{s} along Z\replacedWith{ and (c) along}{-} and X axis. (d) \replacedWith{Magnified}{Zoom in of the green} region marked in \replacedWith{green square}{(c)}.}
...
Traditional integral curve based techniques cannot be used to extract fiber bundle traces from the reliable Hessians because of the spurious nature of the Hessian based local orientation. Thus instead of building fiber traces, we wish to find an abstract representation of the fibers. We build on the key assumptions on the data, namely ``local orientation" and ``connectivity" while taking into account the noise and lack of resolution. We do this by interpreting the underlying geometric structure of the fibers as a set of \removed{polylines(cylinders)} cylinders.
 MetaTracts  are a coarse and simple approximation of integral curves in the form of a continuous chain of cylindrical tubes in $\mathbb{R}^3$ traversing the fiber bundles embedded in the input data. MetaTracts share the following \textit{properties}:

\begin{enumerate}[noitemsep]
\item{\replacedWith{Each MetaTract is}{MetaTracts are} associated with a continuous set of cylinders.}
\item{\replacedWith{Each MetaTract is}{MetaTracts are} associated with a start point at a grid vertex.}
\item{Individual cylinders in MetaTracts have constant lengths, radii, and start points (which are also grid's vertices).}
\item{Individual cylinders in MetaTracts (except the first one) are connected to their previous cylinders at their start point.}
...
\begin{figure}[tb]
 ...
 \includegraphics[ width=\linewidth]{images_pvis/mt_gen.pdf}
  \end{center}
  \vspace{-1em}
 \caption{MetaTracts generation: (b) \replacedWith{MetaTract generation, region with $R_H$ less than threshold}{Generation process}. (a) \replacedWith{Close up of}{Zoom in of the white} marked region in (b). (c) \replacedWith{Parts of a single MetaTract}{Individual cylinder}.}
  \label{fig:algo}
    %\vspace{-1.5em}
    \vskip-0.2cm
\end{figure}
The set of vertices which are located in the green region but not in the blue one are potential \textit{``candidate vertices"} for the start point of the next cylinder in a MetaTract. From these we select another grid vertex which will be the start point for the next cylinder.
%\begin{figure}
%\centering
...
\replacedWith{The output of the above step is a set of MetaTracts. The next step is to cluster the MetaTracts into bundles.}
{The set of MetaTracts generated by the previous step needs to be clustered in order to extract the final fiber bundles. }
\replacedWith{Carbon fibers hav}{Carbon fibers show} both orientation and geometric proximity information, both of which can be used for clustering. Experimentally we found that the orientation measure was very reliable, while measures of geometric proximity had problems with \replacedWith{fibers which partially overlap}{partially overlapping fibers} (Figure~\ref{fig:length_distribution}b). 
...
This step divides the individual MetaTracts into classes based on their major orientations. In order to cluster MetaTracts going in the same directions, we use a spectral embedding technique called Laplacian eigenmaps \replacedWith{which was}{as} originally introduced by Belkin and Niyogi \cite{Belkin01}. An eigenvalue problem is solved, which maps the manifold embedded in the graph into a lower dimensional space while preserving the graph structure.
...
Second, if two \replacedWith{fiber bundles}{MetaTracts} were parallel for some of their length and then separated, they would sometimes be clustered into the same fiber bundle.
...
%\end{figure}  
%  
Figure~\ref{fig:comparison}a shows the MetaTracts that are hierarchically clustered directly using proximity alone into 10, 15, and 20 clusters without first performing orientation clustering (ground truth is 11 clusters). As discussed above, single linkage hierarchical clustering in the presence of overlapping fibers tends to create large  incorrect clusters and small (low-cardinality) outliers. When number of clusters ($h$) is 10, two large incorrect clusters are generated and the rest are outliers. As $h$ increases, some appropriate bundles start to form. Even at $h=20$, the top fiber bundles incorrectly cluster together.
%
Figure~\ref{fig:comparison}b shows the MetaTracts, clustered using K-means clustering by proximity alone, after being embedded in a $m$-dimensional (lower) space, without a second hierarchical clustering step into 10, 15, and 20 clusters. In the lower dimensional space the spatial context is lost and fibers which are in reality far away are grouped together. Even at $h=20$, no correct bundles are identified.
\begin{figure}[tb]
	\centering
	\includegraphics[width=\linewidth]{images_pvis/comparison_all.pdf}
	\caption{Applying only (a)hierarchical and (b)\added{dimensionality reduction followed by} K-means clustering methods to MetaTracts for various numbers of clusters \added{(distance measure is minimal directed Hausdorffs.}}
...
%Fig~\ref{fig:kull} (b) shows the results after the MetaTracts extraction. Fig.~\ref{fig:kull} (c) shows the results of the orientation clustering with $K$ set to 2. Note, that the curved fiber bundle has also been detected with the correct orientation. Fig. \ref{fig:kull} (d) and (e) shows the results of hierarchical clustering of \ref{fig:kull} (c) using proximity based clustering. Both the orientation were clustered into 14 classes.
 ...
 %glass
We tested our method on three datasets with varying characteristics. We report results on two \replacedWith{due to lack of space}{here and third in the supplemental section}.  
Dataset \added{D}1 is described in Sec.~\ref{sec:char_data} (Figure~\ref{fig:data-char}). \replacedWith{Dataset 1}{D1} has eleven separate fiber bundles (5 along one orientation and 6 along the other). All of them were identified correctly by MetaTracts. With different cross-section sizes and varying degree of curvature of the bundles, \replacedWith{dataset 1}{D1} is a complicated dataset.
Figure~\ref{fig:orientation_clustering} and Figure~\ref{fig:crop-16-decomp} shows the result of \replacedWith{dataset 1}{D1} decomposed into two orientation clusters with each orientation cluster further decomposed into $h=10$ clusters followed by voxelization and mesh extraction. \removed{MetaTracts correctly identified the fiber bundles.} Note how the thin and curved purple cluster bundle 7 is extracted well (Figure~\ref{fig:orientation_clustering}e, Figure~\ref{fig:crop-16-decomp}d). 

\removed{The hierarchical clustering is robust against over segmentation. For example,}
Figure~\ref{fig:len_dist_crop16} shows the median, minimum and maximum lengths per cluster in orientation 1 (Figure~\ref{fig:orientation_clustering}d) \added{clustered into 10 clusters, the ground truth being  5 clusters}. We generate the correct clusters and outlier bundles with very few elements which can be discarded. As noted in Sec.~\ref{subsec:dist_clustering}, the MetaTracts do not run the entire length of the dataset, the median length of MetaTracts in cluster six (Figure~\ref{fig:len_dist_crop16}) is 200 while the maximum MetaTract length in the bundle is about 500 (measured in units of grid cube length: 2 $\mu m$). Figure~\ref{fig:length_distribution}b shows the length per MetaTract distribution for a particular fiber bundle. 
Figure~\ref{fig:prepreg} shows the results on our dataset 2 \added{(D2)}. This dataset has different characteristics, and is dense with \textit{flat and thin} bundles; it consists of 4 bundles along one direction and 5 along the other. The dimensions are 300$\times$350$\times$300 and the data type is uint16 (\replacedWith{dataset 1}{D1} is uint8). The bundles are indistinguishable in the original data set Figure~\ref{fig:prepreg}a. A green dotted line shows one of the fiber bundle boundary. Figure~\ref{fig:prepreg}b shows the fiber bundles for the two orientations, Figure~\ref{fig:prepreg}c shows the combined results, and Figure~\ref{fig:prepreg}d shows the volume after the voxelization.

\removed{The fiber bundle extraction using }MetaTracts \replacedWith{was}{are} implemented in C++ using ITK. \removed{All }Clustering \replacedWith{was}{is} done in R \added{(free software environment for statistical computing and graphics)}. The preprocessing is time consuming and may take several hours. It consists of MetaTract generation and the distance computation. The orientation clustering is robust and done as a part of preprocessing. The hierarchical clustering by itself, on the results of the orientation clusters can be done within an order of a few minutes on Intel Xeon E5-2667 workstation and can be repeated according to user input of $h$ (Sec.~\ref{sec:param_choices}). We store the normalized direction at each fiber point, thus both distance measures can be extracted simultaneously without any extra computations.
...
\begin{figure}[tb]
	...
	\vskip-0.2cm
\end{figure}
Four NDT practitioners \replacedWith{who are familiar}{working} with \replacedWith{X-Ray computed tomography}{XCT}\removed{ and its visual representations}, four material scientists \removed{who are} experienced in analyzing \replacedWith{fiber reinforced polymers}{CFRPs} and four \replacedWith{practitioners of visualization}{visualization practitioners} participated in the user evaluation of \replacedWith{our work}{MetaTracts}. We setup a questionnaire with questions on solving given tasks as well as questions using the Likert scale (LS) for evaluating the performance of the MetaTracts over currently used techniques. LS consists of 5 levels from ``strongly disagree" to ``strongly agree". Figure~\ref{fig:userstudy} shows the results \added{in the sequential order they were asked}. \replacedWith{Task 1}{First task}\removed{ of the questionnaire} was to \removed{``}identify the number of fiber bundles\removed{"} using a video\replacedWith{ showing}{of} 360$\degree$ rotation of the\replacedWith{ volume rendering of dataset 1}{D1 volume rendering} (Figure~\ref{fig:data-char}a).\removed{ This task tests the perception of geometric structure and how common problems like uniform grayscale values and occlusion of the different fiber bundles affects the user.}\replacedWith{ Figure~\ref{fig:userstudy} shows that, as expected}{Results show that} the participants found it difficult to determine the number of bundles from volume rendering alone.\replacedWith{ Another common approach is to color map the top eigenvector of Hessian matrix to RGB space. We show a video showing a 360 degree rotation of the volume rendering of the color mapped eigenvectors}{Second task evaluates RGB color mapping of the top Hessian matrix eigenvector. We again show a full 360$\degree$ rotation video where eigenvectors are color-mapped and rendered using volume rendering }\replacedWith{ (similar to}{(as in} Figure~\ref{fig:reliable_hessian}a) and asked the same query. Compared to the volume rendering, the Hessian color map provides more information about the major orientations and \replacedWith{we see}{results demonstrate} that the evaluators performed better than simple volume rendering in this test. The majority of our participants agreed or strongly agreed \removed{(LS)} that Hessian color map was more effective. Finally, we showed the video of bundles extracted with MetaTracts (for orientation 1 and 2 separately Figure~\ref{fig:orientation_clustering}d,e). All participants could correctly identify the number of fiber bundles\replacedWith{. All participants}{ and} agreed that MetaTracts \replacedWith{was}{are} more effective in perceiving geometric structure than simple volume rendering or Hessian color map. They especially liked the \replacedWith{two dimensional}{2D} slices of the MetaTracts (Figure~\ref{fig:crop-16-decomp}b, compared to Figure~\ref{fig:data-char}c).

The next set of tasks evaluated the spatial context perceived from MetaTracts.\replacedWith{ We achieve this by asking the evaluators}{ The participants were asked} to select the fiber bundle which was largest, smallest, or the most similar to a given one from a \removed{sub}set of fiber bundles. Due to the problems of occlusions and similar grayscale values these tests proven to be more difficult to perform successfully with volume rendering of the plain data, compared to MetaTracts. In the Likert scale, the majority strongly agreed that the tasks are undemanding compared to volume rendering.
Finally, the majority of participants agreed that the extracted meshes provides better spatial context than volume rendering. 
...
\acknowledgements{
This work was supported by funding from the European Union’s Seventh Framework Programme (FP7/2007-2013) under grant agreement ACP2-GA-2012-314562-QUICOM. We especially thank Ilya Straumit (KU Leuven) for providing us with dataset 1.
}
